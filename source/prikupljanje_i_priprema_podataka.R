...
### za prosirenje je ukljucivanje ucesnika koji nozda nisu tvitovali u toku 
...
time_narrowed <- startertweets %>% mutate("date" = as.Date(created)) %>% filter(date < "2017-05-27" & date > "2017-05-18")
participants <- unique(time_narrowed$screenName) 

### Za izabrane korisnike su preuzeti dostupni tvitovi pomocu funkcija
### get_tweets i dl_tweets, prvobitno zasnovanih na funkciji ekstrakcija Marka
### Galjaka
### (https://github.com/gljk/Master-rad/blob/master/3.%20Listinzi%20koda.R), ali
### u konacnoj verziji prilicno izmenjenih.
