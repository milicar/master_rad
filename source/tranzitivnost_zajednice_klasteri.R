...
# dodat atribut cvora: broj tvitova o skupu u toku skupa

tweets <- readRDS("results/tweets.RData") %>% filter(conf == TRUE)

nconftws <- tweets %>% distinct(id, .keep_all = TRUE) %>%
...
for(i in 1:length(conf_graph_list)){
        for(j in 1:length(V(conf_graph_list[[i]]))) { 
                V(conf_graph_list[[i]])[j]$nconftws <- as.numeric(nconftws[nconftws$screenName == 
...
lapply(1:length(conf_graph_list), function(y) { 
        filename <- paste0("visuals/2_degree_all_", names(conf_graph_list[y]), ".png")
...
### Jasno je da komunikacija nije ujednacena u celoj mrezi. Na nivou mreze moze
### da se izracuna koeficijent klasterovanja, koji predstavlja odnos trouglova i
...
### koje upucuju na strukturu mreze, kao sto su otkrivanje grupa i klastera i 
### centralizovanost mreze. Ukoliko bi se ispostavilo da je mreza u velikoj meri
...
### neusmerene, uz poruku upozorenja - zbog toga je koriscna neusmerena
### verzija mreza, sa vezom za svaki par uzajamnih veza. Ovde vise nije od
### interesa puka povezanost, vec uzajamna komunikacija, koja ima
### potencijala da opstane i posle skupa. Za izracunavanje klika, kao i za dalje
### analize i vizuelizacije, zgodno je izbaciti izolate.
...
### previsoka; zanemarivanje smera veza bi preuvelicalo ovaj uticaj.
### Centralizacija po bliskosti je srednje visoka ako posmatramo sve veze bez
### obzira na smer (0.64), sto bi znacilo da postoji jedan broj cvorova koji je
### blizi ostalim cvorovima u sveukupnoj komunikaciji. Centralizacija po
...
# klaasterizacija algoritmom walktrap
...
### cvorova). Ipak bez ovog cvora mreza je podeljena na 80 klastera, u odnosu na
### 32 (za isti period i isti algoritam za celu mrezu), sto pokazuje znacaj tog
### cvora za povezivanje grafa. Osim toga, u vizuelizaciji se primecuje drvolika
### struktura, bez znacajnog prisustva klika. Zakljucak je da komunikacija u
### mrezi nije ujednacena, o cemu govore distribucija stepena i srednja, ali
### primetna, centralizovanost mreze (mali broj uticajnih cvorova), nizak
### koeficijent klasterovanja i relativno mali broj klika i velicina jezgara
### (slaba medusobna povezanost "ne-centralnih" cvorova) i veliki broj klastera
### (velika heterogenost mreze).
