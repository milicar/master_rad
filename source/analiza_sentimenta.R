...
#primer tvit wielead #inspire#improve#visibility#enhancement\nwomen for 
#technical leadership @username
sentiment("#innovation") #0.75
sentiment("#enhancement") #0.8
...
#se bas odnosi na sam skup: it's the same BS we always got around the mascot 
#issue at FSU. It's depressing and upsetting #wielead 
...
#Ipak, medu ovima se naslo i nekoliko tvitova koji se odnose bas na govornike i
#predavanja na skupu, na primer: keep writing and deleting tweets abt that last
#presenter. that presentation, esp once I looked up the controversy, really
#upset me #WIELead
...
png("visuals/text1_word_sentiment_contribution.png")
...
### Medu deset najpozitivnijih su se nasle reci "great", "learn",
...
top_words_by_timeframe[[1]] %>% select(word, t_word_sent_by_timefr)

pdf("visuals/text1_word_sentiment_contribution_by_period.pdf")
lapply(1:length(words_sent_by_timeframe), function(x) {
        words_sent_by_timeframe[[x]] %>% group_by(t_word_sent_by_timefr < 0) %>% 
                top_n(10, abs(t_word_sent_by_timefr)) %>%
                ungroup() %>% mutate(word = reorder(word, t_word_sent_by_timefr)) %>% 
                ggplot(aes(word, t_word_sent_by_timefr, fill = t_word_sent_by_timefr < 0)) +
                geom_col() + coord_flip() + 
                labs(title = paste0("doprinos sentimentu\n", periods[x]), 
                     x = NULL, y = "sentiment * broj pojavljivanja") + 
                scale_fill_discrete(name = "", labels = c("pozitivan", "negativan")) +
                theme(plot.title = element_text(face = "bold", hjust = 0.5))
})
        ...
        lapply(1:length(bigrams_sent_by_timeframe), 
               function(x) {
                       w <- bigrams_sent_by_timeframe[[x]] %>% 
                               ...
                               top_n(5, abs(t_big_sent_by_timefr)) %>%
                               ungroup() %>% 
...
lapply(1:length(top_bigrams_by_timeframe), function(x) {
        top_bigrams_by_timeframe[[x]] %>% group_by(t_big_sent_by_timefr < 0) %>% 
                top_n(10, abs(t_big_sent_by_timefr)) %>%
                ungroup() %>% mutate(bigram = reorder(bigram, t_big_sent_by_timefr)) %>% 
                ggplot(aes(bigram, t_big_sent_by_timefr, fill = t_big_sent_by_timefr < 0)) +
                geom_col() + coord_flip() + 
                labs(title = paste0("doprinos bigrama sentimentu\n", periods[x]), 
                     x = NULL, y = "sentiment * broj pojavljivanja") + 
...
### peek", sto nije negativno, "blocks" na "blocks 9am" (nejasno), "building
### blocks" (pozitivno) i "roadblocks" (negativno), a "cad" (kao skracenica) na
### "cad infrastructure" i "spotlight cad", koji nemaju veze sa znacenjem reci
### "cad" - "nitkov, podlac". U trecem periodu, "stop" se odnosi na "to stop"
### (nejasno), "stop by" (ne negativno) i "stop doubting" (pozitivno), a
### "disaster" na "(for) disaster robotics", sto nije negativan pojam, ali se sa
### vrlo negativnom ocenom nasao i u najnegativnijem tvitu. Pozitivni bigrami su
### uglavnom ispravno ocenjeni, osim mozda "machine learning" i "our sponsor"
### koje bih licno ocenila kao neutralne, ali svakako nemaju tako izrazeno
### suprotno znacenje kao neki od navedenih negativno ocenjenih bigrama.
...
### probati sa kontekstualizacijom ovih reči i videti kakva je
### ocena sentimenta ako se ne posmatraju teme skupa. Ipak, teme su u ovom
### slucaju predstavljene vrlo sirokim i opstim terminima, kao sto su ucenje,
### podsticanje, ohrabrenje, pa je moguce da ce se na ovaj nacin neutralisati i
### vise od samih tema skupa.
...
### ovaj paket koristi, ucinilo mi se da i jeste najbolja opcija.
...
### tvitovima nema neke velike promene, svaki od ovih tvitova je i u pocetnoj
### analizi bio medu prvih deset, a i najpozitivniji i najnegativniji tvit je
...
conf_tweets_sentiment2$timeframe <- recode(conf_tweets_sentiments2$timeframe, "time_1" = "Period 1", "time_2" = "Period 2", "time_3" = "Period 3", "time_4" = "Period 4", "time_5" = "Period 5")

png("visuals/distribucija_sentimenta_ridge.png")
...
png("visuals/distribucija_sentimenta_updated_ridge.png")
