...
### Nevezano za odgovor na postavljeno pitanje, u prikaz moze da se doda 
### informacija o tezini ivica, a takodje kao atribut cvorova moze da se ukljuci
### broj tvitova o skupu koje svaki ucesnik ima. Na taj nacin moze da se prati
### gde se nalaze ucesnici koji su najvise tvitovali sa ovim kljucnim recima.
...
### Boje za kodiranje cvorova, od zute do crvene; atribut nconftws ce, posle
### preracunavanja, da odredi indeks boje - svetlozuta za najmanji, crvena za
### najveci broj tvitova
yellowred <- rev(rainbow(26, start = 0, end = 1/6)) 


lapply(1:length(conf_graph_list), function(x) { 
        filename <- paste0("3_01_mreza_ucesnika_sa_brojem_tvitova_za_period_", x, ".png")
        png(filename, width = 860, height = 860)
        g <- conf_graph_list[[x]]
        t <- paste0("Mreža učesnika skupa u periodu ", g$title)
        plot(g, layout = layout_with_fr(g), frame = TRUE, margin = c(0.2, 0, 0, 0),
             vertex.label = NA, 
             vertex.size = 2, 
             vertex.color = yellowred[round(rescale(log(V(g)$nconftws), to = c(0,26)))],
             edge.arrow.size = 0.15, edge.color = alpha("black", rescale(log(E(g)$weight), to = c(0.3, 1))))
        title(t, cex.main = 2)
        legend("bottomright", title = "broj tvitova o skupu", legend = c("veliki broj tvitova", "mali broj tvitova"), 
               cex = 1.5, 
               pt.cex = 2, col = "black", pch = 21, pt.bg = c(yellowred[26], yellowred[1]))
        dev.off() 
})
