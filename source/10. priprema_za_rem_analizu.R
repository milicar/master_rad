...
#tvita sa razlicitim metapodacima o tome koliko je puta retvitovan ili
#favoritovan, nije bas jednostavno kao distinct(df). Prvo treba da se
#grupise, uzme veci broj za ta dva podatka, izbaci originalni broj rt/fav (sa
...
# slicni koraci su odradjeni i u pripremi za analizu teksta, ali su ovde podaci
# koji nisu obradjeni, a osim toga, isto sredjivanje podataka ovde treba da se
# odradi na jos dve varijable.
...
# funkcija zanemarue primaoce/mentione koji nisu posmatrani ucesnici
...
# spaja sve učesnike koji su primaoci poruka iz metapodataka u listu za svaki tvit
...
# onda tvit nije upucen nikome, receiver="none"
# ostavljaju se samo unique primaoci (na primer, ako je primalac pronadjen i 
# u tekstu i kao replyTo ucesnik, zadrzava se samo jedno pojavljivanje)
...
# izbacivanje dogadjaja kod kojih primalac poruke nije medju posmatranim
# ucesnicima ili je isti kao i posiljalac
remove_nondyads_loops <- function(edgelist){
        edgelist <- edgelist %>% filter(receiver != "none" & sender != receiver)
}

# pre izbacivanja self-loops i non-dyads ima 457 posiljalaca, ali posle
# izbacivanja, posiljalaca ima 441, primalaca 182, ukupno 447 razlicitih
# ucesnika

# pravljenje istorije dogadjaja za n najznacajnijih ucesnika:
...
# sentiment je vec izracunat, samo treba da se spoji
# tvitovi koji nisu sa konferencije imaju NA sentiment
